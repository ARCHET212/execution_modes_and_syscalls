...
reset_handler:
    ...
    // 2. Limpiar BSS — BUG FIX: cargar símbolos del linker antes del loop
...
bss_loop:
    ...
    // 3. BUG FIX: En la BBB (AM335x), 0x00000000 es ROM de solo lectura.
    //    Escribir ahí causaba un DATA ABORT inmediato.
    //    La solución correcta es apuntar el VBAR (Vector Base Address Register)
    //    directamente a nuestra vector_table en DDR (~0x82000000).
    ldr r0, =vector_table
